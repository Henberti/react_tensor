...
export const PathFinderLoader = () => {
  return (
    <div className="grid h-screen w-full place-content-center  bg-neutral-950">
      <h1 className="text-white w-3/4 line-clamp-2  lg:text-3xl font-mono">Please Wait while the model is loaded</h1>
      <PathFinder />
    </div>
  );
};
...
export const PathFinder = () => {
  ...
  const bfs = async ({ startTop, startLeft, endTop, endLeft }) => {
    let possiblePaths = [[{ top: startTop, left: startLeft }]];
    let answer = [];
  ...
  const getPossibleNextSteps = ({ top, left }) => {
